...
CGO_ENABLED=0 GOOS=linux go build {gcflags} -o .tiltbuild/bin/{binary} cmd/kueue/main.go
'''.format(repo=repo, binary=binary, gcflags=gcflags),
        deps=['api', 'cmd/{}'.format(binary), 'pkg', 'go.mod', 'go.sum']
    )
...
def deploy_repo(repo, data):
    ...
    for binary, deployment in data['binaries'].items():
        ...
        local_port += 1
    process_yaml(kustomize('../{}/config/default'.format(repo, data['yaml'])))

repo = {
    'image': 'quay.io/vnarsing/kueue',
    'yaml': 'config/',
    'binaries': {
        'manager': 'kueue-controller-manager',
    },
